...
{
  uint8_t tmpBuff[64];
  uint8_t* pBuff = tmpBuff;
  uint16_t i;
  for(i = len; i != 0; i--, buff++, pBuff += 3)
  {
    sprintf((char*)pBuff, "%02X ", *buff);
  }
  printf("\nBuff %d Bytes, HEX:\n%s\n", len, tmpBuff);
}
/* USER CODE END 0 */
